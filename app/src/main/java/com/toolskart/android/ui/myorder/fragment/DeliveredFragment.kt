...
class DeliveredFragment : Fragment(), IAdapterClickListener, MyOrderContract.View {
    ...
    private fun setupOrderListFragmentRecyclerView() {
        ...
        orderListAdapter = BaseRecAdapter(mContext!!, R.layout.items_my_order, adapterClickListener = this, requestType = Constants.DELIVERED_FRAGMENT)
        rv_my_orders.apply {
    ...
    override fun callMyOrder() {
        if (LiveNetworkMonitor(this.mContext!!).isConnected()) {
            ...
            presenter.callMyOrderApi(Constants.DELIVERED)
        } else {
        ...
        if (Validation.isValidObject(data) && data is Order) {
            when (type) {
                ...
                Constants.DELIVERED -> {
                    ...
                    intent.putExtra(Constants.SOURCE, Constants.DELIVERED_FRAGMENT)
