...
class CancelledFragment : Fragment(), IAdapterClickListener, MyOrderContract.View {
    ...
    private fun setupOrderListFragmentRecyclerView() {
        val linearLayout = LinearLayoutManager(mContext, LinearLayout.VERTICAL, false)
        orderListAdapter = BaseRecAdapter(mContext!!, R.layout.items_my_order, adapterClickListener = this, requestType = Constants.CANCELLED_FRAGMENT)
        rv_my_orders.apply {
    ...
    override fun callMyOrder() {
        if (LiveNetworkMonitor(this.mContext!!).isConnected()) {
            ...
            presenter.callMyOrderApi(Constants.CANCELLED)
        } else {
    ...
    override fun onclick(data: Any, pos: Int, type: String, op: String) {
        when (type) {
            Constants.CANCELLED -> {
                ...
                intent.putExtra(Constants.SOURCE, Constants.CANCELLED_FRAGMENT)
